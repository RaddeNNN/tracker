...
public class MainActivity extends AppCompatActivity implements OnLoadCompleteListener {
    ...
    private CountDownTimer timer;//высчитываеся удары в минуту и стучит на каждый тик
...
//        eighthcheck.setChecked(flagsForPlay[counterTool%instruments][7]);
    ...
    private void metronomSetter(){//задает метроном в зависимости от ударов в минуту и количества инстуремнтов (костыльно пока что)
        if(flagsForPlay[0][counterBeat % beats]) mSoundPool.play(soundIdExplosion, 1, 1, 0, 0, 1);
        if(flagsForPlay[1][counterBeat % beats]) mSoundPool.play(soundIdKick,1,1,0,0,1);
        if(flagsForPlay[2][counterBeat % beats]) mSoundPool.play(soundIdSnare,1,1,0,0,1);
